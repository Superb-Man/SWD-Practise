...
const temporarySeatBooking = async (req, res) => {
    ...
    req.body.class_name = 'SHOVAN' ;
    req.body.booked_deatils = [[2,0,,2,1],[2,1,2,1],[2,2,2,1]] ;
    obj = req.body ;
    ...
    try{
        ...
        const dimension_query = {
            text : 'SELECT "train_details".dimensions,"train_details".coaches FROM "train_details" JOIN "train_schedule_info" ON "train_schedule_info".train_id = "train_details".train_id  WHERE schedule_id = $1',
            values : [obj.schedule_id]
        }

        let dimension = -1 ;
        let c_id = -1 ;
        const {dimensions,coaches} = (await trainPool.query(dimension_query)).rows[0] ;
        ...
        for(let i = 0 ; i < obj.booked_deatils.length ; i++) {
            ...
            const query2 = {
                ...
                values : [c_id+1,dimension[1]*obj.booked_deatils[i][0]+obj.booked_deatils[i][1]+1,3,1,obj.schedule_id]
            }
