...
const busPool = require('../config/trainDB');
...
const paymentInitBus = async (req, res) => {

  const { user_name, accesstoken, seat_booked, bus_schedule_id, grandTotalFare, coach_name , seat_booked_string,  transportType , source , destination} = req.body;
  // console.log(ticketInfo);
  console.log(user_name);
  console.log(accesstoken);
  console.log(seat_booked);
  console.log(bus_schedule_id);
  ...
  sslcz.init(data).then(async apiResponse => {
    ...
    try {
      ...
      const getTicketInfoQuery = {
        ...
        values: [ bus_schedule_id, user_name, seat_booked , seat_booked_string, coach_name , transactionId, 'false', details , grandTotalFare]
      }
      const result = await busPool.query(getTicketInfoQuery);
      console.log(result);
...
const paymentSuccessBus =  async (req, res) => {
  ...
  */
  ...
  busPool.query('BEGIN');
  const data = req.body;
  // const { bus_schedule_id  } = req.body;
  console.log(req.body);  
  ...
  try{
      ...
      const result = await busPool.query(updatePaidStatus);
      ...
      const transactionId = req.body.transactionId;
        ...
        const result2 = await busPool.query(getTicketInfo);
        ...
        const getTrainScheduleInfoQuery = {
            text: `SELECT * FROM "bus_schedule_info" WHERE schedule_id = $1`,
            values: [schedule_id]
        };
        const result4 = await busPool.query(getTrainScheduleInfoQuery);
        ...
        const getTrainServicesQuery = {
            text: `SELECT * FROM "bus_services" WHERE train_id = $1`,
            values: [train_id]
        };
        const result5 = await busPool.query(getTrainServicesQuery);
...
const paymentFail = async (req, res) => {
  ...
  */
  ...
  try{
    ...
    // }
    // const selresult = await busPool.query(selectquery).rows[0];

    // console.log(selresult)

    // // const checkQuery ={
    // //   text : 'SELECT * from "bus_schedule_info" WHERE schedule_id = $1 and cancel_deadline >= $2',
    // //   values : [selresult.schedule_id,new Date()]
    // // }

    // // const check = (await busPool.query(checkQuery)).rows[0] ;
    ...
    // let class_id = (await busPool.query(query3)).rows[0].class_id;
    // //console.log(dimensions)
    

    // const dimension_query = {
    //   text : 'SELECT "air_details".dimensions,"air_details".classes FROM "air_details" JOIN "bus_schedule_info" ON "bus_schedule_info".flight_id = "air_details".flight_id  WHERE schedule_id = $1',
    //   values : [selresult.schedule_id]
    // }

    // let {dimensions,classes} = (await busPool.query(dimension_query)).rows[0] ;
    ...
    //   //update two columns
    //   const query2 = {
    //       text : 'UPDATE "bus_schedule_info" SET seat_details[$1][$2][$3] = $4 WHERE schedule_id = $5',
  ...
  const result = await busPool.query(deletequery);
  console.log(result);
